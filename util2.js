var utils = (function () {
    ...
    function event(e) {
        ...
        var pageX=event.pageX||event.clientX+(document.documentElement.scrollLeft || document.body.scrollLeft);
        var pageY=event.pageY||event.clientY+(document.documentElement.scrollTop || document.body.scrollTop);
        var stopPropagation= event.stopPropagation || function () {
           event.cancelBubble = true;
       };
        var preventDefault=event.preventDefault||function(){event.returnValue = false;}
        return{
            event:event,
            target:target,
            pageX:pageX,
    ...
    //$("#demo")  $(".one")  $("div")
     function getEle(str) {
        var s = str.charAt(0);//一个s的变量存放的是符号 # .
        var ss = str.substr(1);
        switch (s) {
            case "#":
                ...
                break;
            case ".":
                return this.getElementsByClass(ss);
                break;
            default :
    ...
    function setGroupCss( options) {
        ...
        for (var key in options) {
            if (options.hasOwnProperty(key)) {//去掉私有属性
    ...
    function css(curEle) {
        //三种情形:
        // 1: css(ele,{width:500,height:400}); 设置一组属性值
        //2: css(ele,"width");//获取属性
        //3:css(ele,"height",300);//设置一个属性值
        var secondParam=arguments[1];
        var thirdParam=arguments[2];
        if(typeof  secondParam==="string"){
            if(typeof thirdParam==="undefined"){//判断第三个参数不存在
               return getCss.apply(curEle,[secondParam]);
            }
            //第三个参数存在
            //debugger;
            setCss.apply(curEle,listToArray(arguments).slice(1));// "300"
        }
        secondParam=secondParam||[];
        if(secondParam.toString()=="[object Object]"){
            setGroupCss.apply(curEle,[secondParam]);
        }
    }
